package com.travel.payment.web;

import com.travel.payment.dto.ErrorResponse;
...
class GlobalExceptionHandlerTest {
    ...
    void handlesResponseStatusException() {
        ResponseEntity<ErrorResponse> response = handler.handleStatus(
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Paiement introuvable"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody().message()).contains("Paiement");
    }
    ...
    void handlesValidationException() {
        ...
        when(bindingResult.getFieldErrors()).thenReturn(
                List.of(new FieldError("request", "amount", "must be positive")));
