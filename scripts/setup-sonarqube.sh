#!/usr/bin/env bash
# Configure SonarQube pour l'audit : projet, token, quality gate
set -euo pipefail

SONAR_URL="${SONAR_URL:-http://localhost:9002}"
SONAR_ADMIN="${SONAR_ADMIN:-admin}"
PROJECT_KEY="${SONAR_PROJECT_KEY:-travel}"
GATE_NAME="${SONAR_QUALITY_GATE:-Travel Audit Gate}"
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ENV_CI_FILE="${ENV_CI_FILE:-$ROOT/.env.ci}"

# Mot de passe : variable env > .env.ci > admin par défaut
...
sonar_check_auth() {
  ...
  echo "  → Connectez-vous sur $SONAR_URL et vérifiez le mot de passe admin."
  echo "  → Relancez avec : SONAR_ADMIN_PASSWORD=votre_mdp ./scripts/setup-sonarqube.sh"
  exit 1
}
...
sonar_api POST "projects/create" -d "project=$PROJECT_KEY&name=Travel Management System" >/dev/null 2>&1 || true
...
echo "=== Réinitialisation conditions audit (bugs=0, vulnérabilités=0) ==="
...
echo "Conditions : bugs > 0 (error), vulnerabilities > 0 (error)"
...
TOKEN=$(sonar_api POST "user_tokens/generate" -d "name=jenkins-travel-$(date +%Y%m%d-%H%M%S)" | python3 -c "import sys,json; print(json.load(sys.stdin)['token'])")
...
echo "  docker compose -f jenkins-compose.yml up -d --force-recreate jenkins"
